...
class Main {
    ...
    public static void bfs() {
        Queue<int[]> Q = new LinkedList<>();    // BFS 구현을 위해 사용. 확인할 칸을 큐에 들어온 순서대로 탐색하기 위함
        Q.add(new int[]{0, 0});     // 이 문제는 지도의 왼쪽 위 끝(0, 0)에서 시작해서 오른쪾 아래 끝(N - 1, M - 1)까지 가는 문제이므로 0,0을 넣고 시작
        // visited의 0은 방문 안했다는 의미로 사용함. 그리고 이 문제에서는 지나온 칸 수를 나타낼것임
        // 문제에서 칸을 셀 때 시작위치와 도착 위치를 포함한다 하여 1을 넣고 거리 측정
        visited[0][0] = 1;

        // while 돌리면서 탐색. Q가 비었다는건 BFS를 위한 탐색을 할 수 있는 모든 칸을 완료했다는 의미
        while(!Q.isEmpty()) {   // 큐가 비면 더 이상 다음 이동이 없다는 의미. visited가 있으므로 큐는 절대 중복된 값이 들어오지 않는다!!!
            int[] pos = Q.poll();   // 큐의 front 값을 저장. 자바에서 poll을 하면 큐의 front 값을 가져오면서 pop도 됨
            int r = pos[0];
            int c = pos[1];
            ...
            for(int i = 0; i < 4; i++ ){    // 전역변수로 선언한 dr, dc 쓰는 부분. 상 하 좌 우 for문으로 확인
                ...
                if(a[nr][nc] == 0) continue;    // 문제 조건에서 0은 갈 수 없는 곳.
    ...
    public static void main(String[] args) throws Exception{
        ...
        for(int i = 0; i < N; i++) {
            String row = br.readLine();    // 한 줄 입력 받음. N번을 입력 받음

            for(int j = 0; j < M; j++) {    // M까지 안하고 row.length()까지 하는것도 방법임.
                // 아스키코드 기준으로 '0' ~ '9'에서 '0'을 빼주면 숫자 0 ~ 9로 알맞게 변환 됨.
                a[i][j] = row.charAt(j) - '0';  // 지도에 각 칸에 숫자 하나씩 넣어 지도를 완성시킴
            }   // end of for j
        }   // end of for i
        bfs();
        System.out.println(visited[N - 1][M - 1]);  // 도착지가 N - 1, M - 1 이었으므로 여기까지 온 칸 수가 visited[N-1][M-1]에 저장되어 있음
    }
}